    ...
    <div>
        <el-container style="position:absolute;top:55px;right:0px;left:0px;bottom:0px">
            <el-header class="d-flex align-items-center">
                ...
                <el-button type="danger" size="small" v-show="chooseImageList.length"
                @click="delImage({all:true})">批量删除</el-button>
                <el-button size="small" v-show="chooseImageList.length"
                    @click="allCancel"
                >全部取消</el-button>
            ...
            <el-footer class="d-flex px-0">
                <div style="width:200px;flex-shrink:0" class="d-flex align-items-center 
        ...
        <!-- 修改相册 | 创建相册 -->
        <el-dialog
            :title=" albumsEditIndex > -1 ? '修改相册信息' : '创建相册'"
...
import imageItem from '@/components/image/index.vue'
    export default{
        ...
        methods:{
            ...
            // 删除图片
            delImage( obj ){
                this.$confirm(`此操作将永久删除${ obj.all ? '选中' : '该' }图片，是否继续?`, '温馨提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                    }).then(() => {
                        // 判断是否是批量删除
                        if( obj.all ){
                            let arr = this.chooseImageList;
                            // 过滤选中的图片
                            this.imageList = this.imageList.filter( item => !arr.some( img => img.id === item.id ) )
                            // 删除成功直接把选中的列表情况
            ...
            // 取消全部选中
            allCancel(){
                this.chooseImageList = this.chooseImageList.filter( item => {
                    this.imageList[item.index].is_click = false
                    return false
                } )
            }
        }
